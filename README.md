...
two scenarios, Opening and Closing, read straight out of the Rhino models and
checked twice: that they fit, and that a driver can get every vehicle back out
through the gate.
...
Opening** and **Scenario 2 Closing** in the header. **Play arrival** fills the
...
Eleven vehicles all day: eight Flagships, HG3 and two HGF bikes. At night they
are all inside. In the morning HG3 and three Flagships pull out to the runway
so the five that stay can open both doors and load together, then leave in any
order.

## The four tight points

**The diagonal's doors overlap by 21 mm, of height rather than of plan.** At
the 3494 mm pitch each open door sweeps about 0.63 m2 over the next vehicle's
...
Nothing touches at body height. So either drop that arm 25 mm, or run the rule
that D2 shuts its north door to let D1 out, D3 for D2, D4 for D3.

**The two HGF bikes clash at the handlebars**, by 78 mm: 651 mm across the
bars on 572 mm centres. Below bar height they are 97 mm apart and fine.

**HG3 and N4 park over the check-in zone**, 0.6 and 0.9 m2 of it, and HG3
stops 101 mm short of its south edge.
...
exactly. The method is rotation invariant, so the 45.24 degree diagonal is as
precise as the square-on rows. HG3 and the two bikes are drawn from the
models' own meshes. Nothing in this repository is fitted or rounded.
...
- `docs/`, the write-up.
- `source/`, the geometry engine, the manoeuvre planner, the mesh packer and
  the exact polygon audit that every figure above comes from.
